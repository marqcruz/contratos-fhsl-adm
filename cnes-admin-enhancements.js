(function(){
  ...
  function isAdmin(){
    try{return !!me && String(me.role||'').toLowerCase()==='admin';}catch(e){return false;}
  }
  ...
  function copyCbo(v){
    ...
    try{
      if(typeof tdngoCopyCnes==='function')return tdngoCopyCnes(code,'CBO',false);
    }catch(e){}
  ...
  function patchCboCopyButtons(){
    document.querySelectorAll('button.tdngo-copy').forEach(function(b){
      ...
      if(title.indexOf('cbo')<0)return;
      b.dataset.tdngoCboOnly='1';
      b.title='Copiar somente o código CBO';
      b.onclick=function(ev){
        ...
        if(tr){
          ...
          var c=ck?findRow(ck.value):null;
          if(c)value=c.CBO||'';
        }
        if(!value){
          var field=document.getElementById('c-cbo');
          if(field&&b.parentElement===field.parentElement)value=field.value||'';
        }
        if(!value){
          var td=b.closest('td');
          value=td?td.textContent:'';
        }
        copyCbo(value);
      };
    });
  ...
  window.editForm=function(id){
    ...
    if(isExcluded(c)&&!isAdmin()){try{toast('Apenas administradores podem editar cadastros excluídos.','err');}catch(e){}return;}
    openForm();
  ...
  window.tdngoDeleteCnesPermanent=async function(id){
    if(!isAdmin()){try{toast('Apenas administradores podem excluir definitivamente.','err');}catch(e){}return;}
    var c=findRow(id);
    var nome=c&&c.Nome?(' de '+c.Nome):'';
    if(!confirm('Excluir DEFINITIVAMENTE o cadastro'+nome+'?\n\nEsta ação remove o vínculo do banco e não poderá ser desfeita.'))return;
    try{
      await post(API,{action:'delete',id:id});
      toast('Cadastro excluído definitivamente.','ok');
      await loadRows();
    }catch(e){toast(e.message||'Não foi possível excluir definitivamente.','err');}
  };
  ...
  function addExcludedAdminActions(){
    ...
    body.querySelectorAll('tr').forEach(function(tr){
      ...
      if(!td.querySelector('.tdngo-edit-excluded')){
        var edit=document.createElement('button');edit.type='button';edit.className='btn sm tdngo-edit-excluded';edit.title='Editar cadastro excluído';edit.textContent='✏️';
        edit.onclick=function(ev){ev.stopPropagation();window.editForm(c.ID);};td.appendChild(edit);
      }
      if(!td.querySelector('.tdngo-delete-permanent')){
        var del=document.createElement('button');del.type='button';del.className='btn sm red tdngo-delete-permanent';del.title='Excluir definitivamente do banco';del.textContent='🗑';del.style.marginLeft='5px';
        del.onclick=function(ev){ev.stopPropagation();window.tdngoDeleteCnesPermanent(c.ID);};td.appendChild(del);
      }
    });
  }
