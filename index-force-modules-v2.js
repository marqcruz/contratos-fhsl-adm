...
function run(){defs.forEach(ensure);applyAllPermissions();count();reveal()}
function hardLogout(){try{sessionStorage.removeItem('fhsl_session');localStorage.removeItem('fhsl_session');sessionStorage.removeItem('tdngo_explicit_nav');sessionStorage.removeItem('tdngo_session_message')}catch{}try{window.currentUser=null}catch{}location.replace('index.html?logout=1&_='+Date.now())}
...
window.addEventListener('pageshow',()=>{if(!session())location.replace('index.html?logout=1&_='+Date.now());else run()});
