  ...
  async function loadHistory(id){
    ...
    try{
      ...
      box.innerHTML='<div class="email-v1-history">'+arr.map(function(h){
        var when=h.enviado_em||h.criado_em,dt=when?new Date(when).toLocaleString('pt-BR'):'—',status=h.status==='ENVIADO'?'<span class="email-v1-ok">✓ Enviado</span>':(h.status==='PENDENTE'?'<span style="color:var(--text3);font-weight:700">… Enviando</span>':'<span class="email-v1-err">✗ Erro</span>');
        ...
        var opens=Number(h.quantidade_aberturas||0),first=h.primeira_abertura_em?new Date(h.primeira_abertura_em).toLocaleString('pt-BR'):'',last=h.ultima_abertura_em?new Date(h.ultima_abertura_em).toLocaleString('pt-BR'):'';
        var openBadge=h.status==='ENVIADO'?(opens?'<span class="email-v1-open">◉ Abertura registrada</span>':'<span style="color:var(--text3)">○ Sem abertura registrada</span>'):'';
        var openLine=opens?'<div><b>Primeira abertura registrada:</b> '+esc(first)+' · <b>Registros:</b> '+opens+(opens>1&&last?' · <b>Última:</b> '+esc(last):'')+'</div>':'';
        return'<div class="email-v1-history-item"><div class="email-v1-history-top"><b>'+esc(typeLabel(h.tipo_comunicacao))+'</b>'+status+openBadge+'<span style="color:var(--text3)">'+esc(dt)+'</span><span style="color:var(--text3)">por '+esc(h.enviado_por_nome||h.enviado_por_email||'—')+'</span></div><div style="margin-top:4px"><b>Para:</b> '+esc(tos)+'</div>'+cc+'<div><b>Assunto:</b> '+esc(h.assunto||'')+'</div>'+openLine+(h.erro?'<div class="email-v1-err">'+esc(h.erro)+'</div>':'')+'<details style="margin-top:5px"><summary style="cursor:pointer;color:var(--accent)">Ver mensagem</summary><div style="white-space:pre-wrap;margin-top:7px;color:var(--text2)">'+esc(h.corpo||'')+'</div></details></div>';
      }).join('')+'</div>';
