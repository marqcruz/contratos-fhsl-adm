...
function removeTicketRows(id){document.querySelectorAll('tr').forEach(tr=>{const link=tr.querySelector('.row-link[onclick]');if(!link)return;const m=String(link.getAttribute('onclick')||'').match(/'([0-9a-f-]{36})'/i);if(m&&m[1]===id)tr.remove()})}
